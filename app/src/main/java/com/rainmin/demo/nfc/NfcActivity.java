...
public class NfcActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private final static String TAG = "Xscape";
    // NFC适配器
    ...
    public void processIntent(Intent intent) {
        ...
        // add card ID
        infoBuilder.append("卡片ID：" + bytesToHexString(tagFromIntent.getId()) + "\n");
        // add supported tech
        infoBuilder.append("支持的技术：" + tagFromIntent.toString() + "\n");

        String[] techList = tagFromIntent.getTechList();
        //分析NFC卡的类型： Mifare Classic/UltraLight Info
        String cardType = "NFC卡类型：";
        ...
        infoBuilder.append(cardType);
        promt.setText(infoBuilder.toString());
    }
    ...
    private String read(Tag tag) throws IOException, FormatException {
        if (tag != null) {
            ...
            if (ndef == null) {
                Log.e(TAG,"tag don't support Ndef format");
                return null;
            }
            //打开连接
            ndef.connect();
    ...
    private void write(Tag tag) throws IOException, FormatException {
        if (tag != null) {
            ...
            if (ndef.getMaxSize() < size) {
                Log.e(TAG, "tag storage is smaller than message");
            }
            // 写入NDEF信息
            ndef.writeNdefMessage(message);
            // 关闭连接
            ndef.close();
            Toast.makeText(NfcActivity.this, "写入数据成功", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(NfcActivity.this, "设备与nfc卡连接断开，请重新连接...",
                    Toast.LENGTH_SHORT).show();
        }
